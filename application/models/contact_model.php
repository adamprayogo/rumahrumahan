...
class contact_model extends CI_Model {
	...
	function get_by_county_id($id) {
		$select = '*';
		$array_where = array('contacts.county_id' => $id);
		$array_like = array();
		$order_by = array();
	...
	function get_by_exact_name($name){
		$select = '*';
		$array_like=array();
		$array_where = array('contacts.name'=>$name);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, $order_by);
	}
	...
	function get_by_name($name, $first, $offset) {
		$select = '*';
		$array_where = array();
		$array_like = array('contacts.name'=>$name);
		$order_by = array();
	...
	function get_by_name_and_diff_id($id,$name){
		$select = '*';
		$array_where = array('contacts.name'=>$name,'contacts.id <>'=>$id);
		$array_like = array();
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, $order_by);
	}
	...
	function get_by_id_and_name($id,$name, $first, $offset) {
		$select = '*';
		$array_where = array();
		$array_like = array('contacts.name'=>$name,'contacts.id'=>$id);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, $first, $offset, $order_by);
	}
