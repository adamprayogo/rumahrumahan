...
class estates_amenities_model extends CI_Model {
	...
	function get($select = "*", $array_where = false, $array_like = false, $first = false, $offset = false, $order_by = false) {
		...
		$this -> db -> from('estates_amenities');
		...
		$this->db->join('amenities','estates_amenities.amenities_id = amenities.id','left');
		$query = $this -> db -> get();
		if ($query -> num_rows() > 0) {
	...
	function get_by_estates_id($estates_id){
		$select = '*';
	...
	function insert($data_array) {
		...
		$this -> db -> insert('estates_amenities', $data_array);
		return $this -> db -> insert_id();
	}

	public function remove($arr_where) {
		$this -> db -> where($arr_where);
		$this -> db -> delete('estates_amenities');
		return $this->db->affected_rows();
	}

	public function remove_by_estates_id($id) {
	...
	function update($data_array, $array_where) {
		$this -> db -> where($array_where);
		$this -> db -> update('estates_amenities', $data_array);
	}
}
?>