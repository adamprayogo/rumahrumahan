...
class packages_model extends CI_Model {
	...
	function get_by_exact_title($name){
		$select = '*';
		$array_like=array();
	...
	function get_by_title($name, $first, $offset) {
		$select = '*';
		$array_where = array();
	...
	function get_by_title_and_diff_id($id,$name){
		$select = '*';
		$array_where = array('title'=>$name,'id <>'=>$id);
		$array_like = array();
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, $order_by);
	}

	function get_by_id_and_title($id,$name, $first, $offset) {
		$select = '*';
		$array_where = array();
		$array_like = array('title'=>$name,'id'=>$id);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, $first, $offset, $order_by);
	}
	...
	function insert($data_array) {
		...
		$this -> db -> insert('packages', $data_array);
	...
	function update($data_array, $array_where) {
		$this -> db -> where($array_where);
		$this -> db -> update('packages', $data_array);
	}
}
?>