...
class page_model extends CI_Model {
	...
	function get_by_exact_title($title){
		$select = '*';
		$array_like=array();
		$array_where = array('title'=>$title);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, $order_by);
	}

	function get_by_title($title, $first, $offset) {
		$select = '*';
		$array_where = array();
		$array_like = array('title'=>$title);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, $first, $offset, $order_by);
	}

	function get_by_title_and_diff_id($id,$title){
		$select = '*';
		$array_where = array('title'=>$title,'id <>'=>$id);
		$array_like = array();
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, $order_by);
	}

	function get_by_id_and_title($id,$title, $first, $offset) {
		$select = '*';
		$array_where = array();
		$array_like = array('title'=>$title,'id'=>$id);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, $first, $offset, $order_by);
	}
	...
	function insert($data_array) {
		...
		$this -> db -> insert('page', $data_array);
	...
	function update($data_array, $array_where) {
		$this -> db -> where($array_where);
		$this -> db -> update('page', $data_array);
	}
}
?>