...
class partner_model extends CI_Model {
	...
	function get($select = "*", $array_where = false, $array_like = false, $first = false, $offset = false, $order_by = false) {
		...
		/*$this->db->join('users','partner.user_id=users.id');*/
		$query = $this -> db -> get();
		if ($query -> num_rows() > 0) {
	...
	function get_by_id($id) {
		$select = '*';
		$array_where = array('partner.id' => $id);
		$array_like = array();
		$order_by = array();
	...
	function get_by_parent_id($id) {
		$select = '*,partner.id as id,partner.avt as avt,partner.updated_at as updated_at,partner.created_at as created_at,title,partner.activated as activated';
		$array_where = array('partner.parent_id' => $id);
		$array_like = array();
		$order_by = array();
	...
	function get_by_exact_name($name){
		$select = '*,partner.id as id,partner.avt as avt,partner.updated_at as updated_at,partner.created_at as created_at,title,partner.activated as activated';
		$array_like=array();
		$array_where = array('partner.title'=>$name);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, $order_by);
	}
	...
	function get_by_name($name, $first, $offset) {
		$select = '*,partner.id as id,partner.avt as avt,partner.updated_at as updated_at,partner.created_at as created_at,title,partner.activated as activated';
		$array_where = array();
		$array_like = array('partner.title'=>$name);
		$order_by = array();
	...
	function insert($data_array) {
		...
		$this -> db -> insert('partner', $data_array);
	...
	function update($data_array, $array_where) {
		$this -> db -> where($array_where);
		$this -> db -> update('partner', $data_array);
	}
}
?>