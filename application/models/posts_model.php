...
class posts_model extends CI_Model {
	...
	function get_by_id($id) {
		$select = '*,posts.id as id,posts.avt as avt,posts.updated_at as updated_at,posts.created_at as created_at,title,posts.activated as activated';
		$array_where = array('posts.id' => $id);
		$array_like = array();
		$order_by = array();
	...
	function get_by_parent_id($id) {
		$select = '*,posts.id as id,posts.avt as avt,posts.updated_at as updated_at,posts.created_at as created_at,title,posts.activated as activated';
		$array_where = array('posts.parent_id' => $id);
		$array_like = array();
		$order_by = array();
	...
	function get_by_exact_name($name){
		$select = '*,posts.id as id,posts.avt as avt,posts.updated_at as updated_at,posts.created_at as created_at,title,posts.activated as activated';
		$array_like=array();
		$array_where = array('posts.title'=>$name);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, $order_by);
	}
	...
	function get_by_name($name, $first, $offset) {
		$select = '*,posts.id as id,posts.avt as avt,posts.updated_at as updated_at,posts.created_at as created_at,title,posts.activated as activated';
		$array_where = array();
		$array_like = array('posts.title'=>$name);
		$order_by = array();
