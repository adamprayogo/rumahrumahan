...
class top_slider_model extends CI_Model {
	...
	function get_by_id($id) {
		$select = '*';
		$array_where = array('top_slider.id' => $id);
		$array_like = array();
		$order_by = array();
	...
	function get_by_parent_id($id) {
		$select = '*';
		$array_where = array('top_slider.parent_id' => $id);
		$array_like = array();
		$order_by = array();
	...
	function get_by_exact_name($name){
		$select = '*';
		$array_like=array();
		$array_where = array('top_slider.name'=>$name);
		$order_by = array();
		return $this -> get($select, $array_where, $array_like, 0, 1, $order_by);
	}
	...
	function get_by_name($name, $first, $offset) {
		$select = '*';
		$array_where = array();
		$array_like = array('top_slider.name'=>$name);
		$order_by = array();
	...
	function insert($data_array) {
		...
		$this -> db -> insert('top_slider', $data_array);
		return $this -> db -> insert_id();
	}

	public function remove($arr_where) {
		$this -> db -> where($arr_where);
		$this -> db -> delete('top_slider');
	...
	function update($data_array, $array_where) {
		$this -> db -> where($array_where);
		$this -> db -> update('top_slider', $data_array);
	}
}
?>