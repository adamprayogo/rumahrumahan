...
class amenities extends Backend_Controller{
	function __construct()
	{
		parent::__construct();
		$this->load->model('amenities_model');
		$this->bk_menu=6;
		$this->bk_title=$this->lang->line('msg_amenities');
	}

	function index(){
		$base_url=base_url().'admin/amenities';
		$page=$this->uri->segment(3);
		if(!is_numeric($page) || $page<=0){
			$page=1;
		}
		$first=($page-1)*$this->pg_per_page;
		$total_rows=$this->amenities_model->total(array(),array());
		
		$data['list'] = $this->amenities_model->get("*,", array(),array(),$first, $this->pg_per_page, array('id' => 'DESC'));
		$data['page_link'] =parent::pagination_config($base_url,$total_rows,$this->pg_per_page);
		$data['heading']=$this->lang->line('msg_amenities');
		$this->render_backend_tp('backends/amenities/index',$data);
	}
		...
		if(isset($_POST['name'])){
			...
			if($this->form_validation->run()!=false){
				...
				if($insert_id!=0){
					...
					redirect(base_url().'admin/amenities/create');
				}
			}
		}
		$data['heading']=$this->lang->line('msg_add');
		$this->load->model('amenities_model');
		$this->render_backend_tp('backends/amenities/add',$data);
	}

	public function check_name_exist_add($name){
		$data=$this->amenities_model->get_by_exact_name($name, 0, 1);
		if ($data!=null)
		{
	...
	public function check_name_exist_edit(){
		...
		$data=$this->amenities_model->get_by_name_and_diff_id($id,$name);
		if($data!=null) {
		...
		if(isset($_GET['id'])){
			...
			if(isset($_POST['id_post'])){
				...
				if($this->form_validation->run()){
					$this->amenities_model->update(array('name'=>$name),array('id'=>$id));
					$this->session->set_flashdata('msg_ok',$this->lang->line('edit_successfully'));
					redirect(base_url().'admin/amenities/edit?id='.$id);
				}
			}
			$data['obj']=$this->amenities_model->get_by_id($id);
			if($data['obj']==null){
				redirect('notfound');
			}
			$data['heading']=$this->lang->line('msg_edit');
			$this->render_backend_tp('backends/amenities/edit',$data);
		}else{
			redirect('notfound');
		}
	}
	...
	public function delete(){
		if(isset($_GET['id'])){
			$id=$this->input->get('id');
			$this->amenities_model->remove_by_id($id);
	...
	public function search(){
		if(isset($_GET['query'])){
			...
			$config['total_rows'] = $this->amenities_model->total(array(), array('name'=>$query));
			$config['base_url']= base_url() . 'admin/amenities/search?order='.$order.'&query='.$query;
			$config['per_page']=$per_page;
			$data['msg_label']=$this->config->item('msg_label');
			$this->pagination->initialize($config);
			$data['list'] = $this->amenities_model->get_by_name($query,$page,$per_page);
